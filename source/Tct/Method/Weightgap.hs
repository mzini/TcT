...
applyWeightGap :: P.SolverM m => Trs.Trs -> UsablePositions -> Trs.Trs -> Prob.StartTerms -> F.Signature -> NaturalMIKind -> PolyCheck -> Nat -> N.Size -> Maybe Nat -> Bool
               -> m (OrientationProof MatrixOrder)
applyWeightGap nondup uarg trs st sig mk pcheck d b cb ua = orientMatrix (weightGapConstraints nondup) uarg' st trs Trs.empty sig (mk :+: pcheck :+: d :+: (Nat $ N.bound b) :+: Nothing :+: cb :+: ua)
