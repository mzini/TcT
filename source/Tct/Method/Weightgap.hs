...
applyWeightGap uarg nondup trs st sig mk d b cb 
    | Trs.isDuplicating nondup = return $ Inapplicable "Trs duplicating"
    | otherwise                = orientMatrix (weightGapConstraints uarg nondup) st trs Trs.empty sig (mk :+: d :+: (nat $ N.bound b) :+: Nothing :+: cb)
